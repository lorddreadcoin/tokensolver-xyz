...
export const config = { path: "/score" };
...
export default async (req) => {
  try {
    ...
    if (!kind || !address) return new Response(JSON.stringify({ error: "Missing kind or address" }), { status: 400 });

    if (kind === "wallet") {
      const out = await scoreWallet(address);
      return Response.json(out);
    } else if (kind === "token") {
      const out = await scoreToken(address);
      return Response.json(out);
    } else {
      return new Response(JSON.stringify({ error: "Unknown kind" }), { status: 400 });
    }
  } catch (err) {
    return new Response(err.message || "Failed to score", { status: 500 });
  }
};
